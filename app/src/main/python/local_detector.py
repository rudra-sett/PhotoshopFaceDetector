...
def detect_changes(model_path, img_path, no_crop=False):
    # img_path = args.input_path
    # dest_folder = args.dest_folder
    # model_path = args.model_path
    # gpu_id = args.gpu_id
    dest_folder = "/storage/emulated/0/DCIM"
    # Loading the model
    if torch.cuda.is_available():
        device = 'cuda:{}'.format(gpu_id)
    else:
    ...
    model = DRNSeg(2)
    print("loading model...)")
    #model_path = io.BytesIO(model_path)
    state_dict = torch.load(io.BytesIO(model_path), map_location=device)
    print("loaded model!")
    model.load_state_dict(state_dict['model'])
    model.to(device)
    model.eval()
    ...
    if no_crop:
        face = Image.open(io.BytesIO(img_path)).convert('RGB')
    else:
        faces = face_detection(img_path, verbose=False)
        if len(faces) == 0:
            print("no face detected by dlib, exiting")
    ...
    #    os.path.join(dest_folder, 'warped.jpg'),
    #    quality=90)
    reverseba = io.BytesIO()
    reverse.save(reverseba, format='PNG')
    reverseba = reverseba.getvalue()

    flow_magn = np.sqrt(flow[:, :, 0]**2 + flow[:, :, 1]**2)
    heatba = save_heatmap_cv(
        modified_np, flow_magn,
        os.path.join(dest_folder, 'heatmap.jpg'))
    return heatba
